...
#### **Case 3: Predy Finance – Exploiting Callback to Withdraw Funds**
...
*/
function _mint(address .sender, uint256 .amount) internal {
    (,uint256 uniBTCAmount) = _amounts(.amount);
    require(uniBTCAmount > 0, MUSR010");
    require(address(this).balance <= caps[NATIVE.BTC], MUSR003M);
    IMintableContract(uniBTC)・mint(_sender, uniBTCAmount);
    emit Minted(NATIVE_BTC, .amount);
}
/**
* @dev mint uniBTC with wrapped BTC tokens
 */
function _mint(address .sender, address .token, uint256 _amount) internal {
    (,uint256 uniBTCAmount) = _amounts(.token, .amount);
    require(uniBTCAmount > % MUSR010");
    require(IERC20(token) .balanceOf (address( this)) + amount <= caps[tokenL ''USR003”);
    IERC20(_token).safeTransferFrom(_sender, address(this)f
    .amount);
    IMintableContract(uniBTC)・mint(.sender, uniBTCAmount);
    emit Minted(_token, .amount);
}
...
public {
    if(agentWallets[agent] address(0)){
        ...
        IERC20Upgradeable(token).safeTransferFrom(fromr
        agentWallets[agent],
        amount);
    }
    else
    {
        ...
        IERC20Upgradeable(token).safeTransferFrom(from, address(this)r
        amount);
...
- **Fund Movement**: The stolen funds were transferred to a **Binance address**.
...
function getPrice() public view returns (uint256) {
    uint256 usdtBalance = IERC20(usdtAddress).balanceOf(IpAddress);
    uint256 HFLHBalance = IERC20(HFLHAddress).baLanceOf(IpAddress);
...
- **Fund Movement**: The stolen funds were distributed across multiple external accounts (EOAs), then bridged from Arbitrum to Ethereum via **LayerZero Stargate** and **Synapse**. Ultimately, the assets were laundered through **Tornado Cash**, totaling **2,120 ETH**.

<details><summary>Code</summary>

```solidity

function _calcQuoteAmountSe\lBase(
...
- **Fund Movement**: The stolen **9.1 ETH** was laundered through **Tornado Cash**.
...
- **Fund Movement**: After negotiations with the project team failed (a bounty of **$120,000** was offered but declined), the attacker laundered the stolen funds via **Tornado Cash**.
...
- **Fund Movement**: The stolen funds were split across more than **50 EOA addresses** to obfuscate the trail. Using Hop Protocol, Synapse, and LayerZero Stargate, the funds were bridged from **Arbitrum to Ethereum** and laundered via **Tornado Cash**.
...
* @param a Ray
* (dpa ram b Ray
* (dreturn The result of a/b, in ray
**/

function rayDiv(uint256 at, uint256 bt) internal pure returns (uint256) {
    require(bt != 0, Errors.MATH_DIVISION_BY_ZERO);
    uint256 halfB = bt / 2;
    require(at <= (type(uint256)-max - halfB) / RAY, Errors.MATH_MULTIPLICATION_OVERFLOW);
    return (at * RAY + halfB) / bt;
}

/**
* @dev Mints 'amount' aTokens to 'user'
* 一 Only callable by the LendingPool, as extra state updates there need to be managed
* @para/n user The address receiving the minted tokens
* @para/n amount The amount of tokens getting minted
* ©param index The new liquidity index of the reserve
* ^return 'true' if the the previous balance of the user was 0
*/
function mint(address usert, uint256 amountt, uint256 indext) external override onlyLendingPool returns (bool) {
    uint256 previousBalance = super.balanceOf(usert);
    uint256 amountScaled = amountt. rayDiv( indext);
    require(amountScaled != 0, Errors.CT_INVALID_MINT_AMOUNT);
    _mint(usert, amountScaled);
    emit Transfer(address(0), usert, amountt);
    emit MinKusert, amountt, indext);
...
- **Fund Movement**: Over the course of **50 days**, the stolen assets were laundered through **Tornado Cash**, amounting to **2,737.5 ETH**.
...
function repayForAlKuint128 amount, bool skim) public returns (uint256) {
    accrueO;
    if (skim) {
    ...
    bentoBox.transfer(magicInternetMoney, msg.sender, address(this), bentoBox.toShare(magiclnternetMoney, amount<|pad|>true));
    uintl28 previousElastic = totalBorrow.elastic;
    require(previousElastic - amount > 1000 * lel8, "Total Elastic too small");
    totalBorrow.elastic = previousElastic - amount,
    enit LogRepayForAll anountf, previousElastic, totalBorrow.elastic);
    return amount;
...
function toElasticRebase memory total, uint256 base, bool roundUp) internal pure returns (uint256 elastic) {
if (total.base = 0 {
    elastic = base;
} else {
...
**Fund Flow**: The stolen funds were bridged to Ethereum via **Across Protocol** over 19 days and later laundered using **Tornado Cash**, with 154 ETH being cleaned.

<details><summary>Code</summary>

```solidity

function _burn(bytes32 key, address user, uint256 burnA«ount)
    public
    selfOnly
    returns (uint256 withdrawal, uint256 withdrawals)
{
    ...
    emit Burn(user, key, withdrawalA, withdrawals, burnAmount);
    IBookManager.BookKey memory bookKeyA = bookManager.getBookKey(pool.bookldA);

    pool. reserveA = _settleCurrency(bookKeyA.quote, reserveA) - withdrawal;
    pool.reserveB = _settleCurrency(bookKeyA.base, reserveB) - withdrawals;

    if (withdrawalA > 0) {
        bookKeyA.quote.transfer(user9
    withdrawalA);
    }
    if (withdrawalB > 0) {
...
**Fund Flow**: Within two days, the stolen funds were bridged to Ethereum via **LayerZero Stargate** and laundered through Tornado Cash, resulting in a total of 410.2 ETH being cleaned.
...
function collectFees(
    uint256 lockld
)external IsLockOwner(lockld) validLockLPv3(lockId) returns (uint256 amounte, uint256 amountl) {
    Lock storage userLock = .locks[lockld];
    // set amountOMax and amountlMax to uint256・max to collect all fees
    // alternatively can set recipient to msg.sender and avoid another transaction in sendToOwner
    INonfungiblePositionManager.CollectParams
        memory params = INonfungiblePositionManager.CollectParams({
        tokenld: userLock.nftld,
        recipient: address(this),
        amountOMax: type(uintl28).max,
        amountlMax: type(uintl28).max
        });
    ...
    uint256 originalAmounte = IERC2e(tokenO).balanceOf(address(this));
    uint256 originalAmountl = IERC20(tokenl).balanceOf(add ress(th is));
    INonfungibLePositionManager(userLock.nftManager).collect(params);
    amounte = IERC20(token0).balanceOf(address(this)) - originalAmounte；
    amountl = IERC20(tokenl).balanceOf(address(this)) - originalAmountl;
    IERC20(token0).safeTransfer(userLock.owner, amounte);
    IERC20(tokenl).safeTransfer(userLock.owner, amountl);
}
...
**Fund Flow**: The stolen funds were laundered using **Tornado Cash** and **Railgun**.
...
**Fund Flow**: The funds were bridged to Ethereum via **Orbiter Finance** and processed further through **CoinsPaid** and **Revolut**.
...
**Fund Flow**: The attacker stole **420.4 BNB** and laundered it through Tornado Cash.
...
**Fund Flow**: An MEV white-hat hacker intercepted part of the attack, recovering some funds. However, attackers managed to launder a significant portion through Tornado Cash and deposited some tokens into exchanges like Bybit.
...
- **Fund Flow**: All stolen funds were returned to the project’s treasury on the same day as the attack.
...
- **Exploitation:** Tokens like TUSD utilize multiple entrypoint contracts that share the same balance information. The main contract is used for primary interactions such as `transfer` and `balanceOf`, while auxiliary entry contracts act as proxies forwarding calls to the main contract. The `sweepToken` function checks whether the token address matches the `underlying` address. Attackers can pass the address of an auxiliary entry contract instead of the main contract, bypassing the check. As a result, the `sweepToken` function processes the transaction and transfers all underlying assets (e.g., TUSD) to the admin's address.

- **Consequences:** The removal of underlying assets from the contract causes the exchange rate (e.g., TUSD/cTUSD) to plummet. Attackers can profit by liquidating other users at a reduced cost and repaying loans at a lower expense.
